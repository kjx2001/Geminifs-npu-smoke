...
constexpr int GPU_file_nums = 4096;
...
int main(int argc, char **argv)
{
    ...
    const int num_blocks = 1024; // Total blocks to fill 32GB for K and 32GB for V cache
    ...
    std::vector<torch::Tensor> k_tensors(num_layers);
    std::vector<torch::Tensor> v_tensors(num_layers);
    for (int layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
        k_tensors[layer_idx] = torch::randn({num_blocks, num_kv_heads, head_dim, block_size}, options);
        v_tensors[layer_idx] = torch::randn({num_blocks, num_kv_heads, head_dim, block_size}, options);
    }
    ...
    size_t reg_granularity = static_cast<size_t>(block_size) * num_kv_heads * head_dim * k_tensors[0].element_size(); // reg the KV cache per layer

    geminifs.geminifs_register_tensors_with_gpu(k_tensors, reg_granularity);
    geminifs.geminifs_register_tensors_with_gpu(v_tensors, reg_granularity);

    // --- Simulate Layer-wise Cache Slicing ---
    const int total_blocks = num_blocks * num_layers;

    int transfer_files = 1024;   // number of blocks/files per batched transfer
    ...
    for (int layer_idx = 0; layer_idx < transfer_layers; ++layer_idx) {
        ...
        for (int block_idx = 0; block_idx < transfer_files; ++block_idx) {
            const int64_t flat_index = block_idx;
            TORCH_CHECK(flat_index < num_blocks,
                        "Requested block index exceeds allocated tensors");
            auto k_slice = k_tensors[layer_idx].slice(0, flat_index, flat_index + 1);
            auto v_slice = v_tensors[layer_idx].slice(0, flat_index, flat_index + 1);
