...
bool NvmeServiceServer::handleClient(int client_fd, ServiceState& state) {
    ...
    if (cmd == Command::Ping) {
        // no payload
    } else if (cmd == Command::AdminInit && role == Role::Admin) {
        if (payload.size() < sizeof(AdminInitReq)) {
            resp.status = static_cast<uint16_t>(Status::Invalid);
        } else {
            const auto* req = reinterpret_cast<const AdminInitReq*>(payload.data());
            const auto* ctrls = reinterpret_cast<const CtrlConfig*>(payload.data() + sizeof(AdminInitReq));
            if (payload.size() < sizeof(AdminInitReq) + req->ctrl_count * sizeof(CtrlConfig)) {
                resp.status = static_cast<uint16_t>(Status::Invalid);
            } else {
                state.applyAdminInit(*req, ctrls, req->ctrl_count);
            }
        }
    } else if (cmd == Command::AdminSetLimits && role == Role::Admin) {
        if (payload.size() < sizeof(AdminSetLimitsReq)) {
            resp.status = static_cast<uint16_t>(Status::Invalid);
        } else {
            const auto* req = reinterpret_cast<const AdminSetLimitsReq*>(payload.data());
            state.max_queues_per_process = req->max_queues_per_process;
        }
