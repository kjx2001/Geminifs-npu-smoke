...
struct tlb_entry {
    ...
    void init() {
        global_id = 0;
        state.store(0, cuda::memory_order_relaxed);
        page = nullptr;
    }
...
//			printf("--(2)st: %llx\tcount: %llu\n", (unsigned long long) state.load(cuda::memory_order_relaxed), (unsigned long long) count);

        state.fetch_sub(count, cuda::memory_order_release); }

    __forceinline__
    __device__
    void release() { if (page != nullptr)  {
//		    if (global_id == 515920192)
//			printf("--(1)st: %llx\tcount: %llu\n", (unsigned long long) state.load(cuda::memory_order_relaxed), (unsigned long long) 1);

            page->state.fetch_sub(1, cuda::memory_order_release); }}
    ...
    T* acquire(const size_t i, const size_t gid, size_t& start, size_t& end, range_d_t<T>* range, const size_t page_) {
        ...
        if (lane == master) {
            ...
            do {
                ...
                //lock;
                do {
                    st = entry->state.fetch_or(VALID_, cuda::memory_order_acquire);
                    if ((st & VALID_) == 0)
                        break;
                    __nanosleep(100);
                } while (true);
...
//			printf("++(1)st: %llx\tst&Val: %llx\tcount: %llu\n", (unsigned long long) st, (unsigned long long) (st & VALID_), (unsigned long long) count);
                    ...
                    entry->state.store(st, cuda::memory_order_release);
                    break;
                }
...
//
                    if (entry->page != nullptr)
                        entry->page->state.fetch_sub(1, cuda::memory_order_release);
                    ...
                    entry->state.store(st, cuda::memory_order_relaxed);
                    __nanosleep(100);
...
//            (unsigned long long) i, (unsigned long long) page, (unsigned long long) start, (unsigned long long) end, (unsigned long long) range_id);
    ...
    T& operator[](const size_t i) {
        if ((i < start) || (i >= end)) {
            update_page(i);
            page->state.fetch_or(DIRTY, cuda::memory_order_relaxed);
        }
        return addr[i-start];
    }
};
...
void __flush(page_cache_d_t* pc) {
    ...
    if (page < pc->n_pages) {
        ...
        uint32_t expected_state = pc->ranges[previous_range][previous_address].state.load(cuda::memory_order_relaxed);
...
void range_d_t<T>::release_page(const size_t pg) const {
    uint64_t index = pg;
    pages[index].state.fetch_sub(1, cuda::memory_order_release);
}
...
void range_d_t<T>::release_page(const size_t pg, const uint32_t count) const {
    uint64_t index = pg;
    pages[index].state.fetch_sub(count, cuda::memory_order_release);
}
...
void range_d_t<T>::mark_page_dirty(const size_t index) {
    pages[index].state.fetch_or(DIRTY, cuda::memory_order_relaxed);
}
...
uint64_t range_d_t<T>::acquire_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue) {
    ...
    //access_cnt.fetch_add(count, cuda::memory_order_relaxed);
    access_cnt.fetch_add(count, cuda::memory_order_relaxed);
    bool fail = true;
    unsigned int ns = 8;
    //bool miss = false;
    ...
    read_state = pages[index].state.fetch_add(count, cuda::memory_order_acquire);
    do {
        ...
        switch (st) {
            //invalid
        case NV_NB:
            st_new = pages[index].state.fetch_or(BUSY, cuda::memory_order_acquire);
            if ((st_new & BUSY) == 0) {
                ...
                //uint32_t ctrl = cache.ctrl_counter->fetch_add(1, cuda::memory_order_relaxed) % (cache.n_ctrls);
                uint64_t ctrl = get_backing_ctrl(index);
                if (ctrl == ALL_CTRLS)
                    ctrl = cache.ctrl_counter->fetch_add(1, cuda::memory_order_relaxed) % (cache.n_ctrls);
                //ctrl = ctrl_;
                uint64_t b_page = get_backing_page(index);
                Controller* c = cache.d_ctrls[ctrl];
                c->access_counter.fetch_add(1, cuda::memory_order_relaxed);
                //uint32_t queue = (tid/32) % (c->n_qps);
                //uint32_t queue = c->queue_counter.fetch_add(1, cuda::memory_order_relaxed) % (c->n_qps);
                //uint32_t queue = ((sm_id * 64) + warp_id()) % (c->n_qps);
                read_io_cnt.fetch_add(1, cuda::memory_order_relaxed);
                read_data(&cache, (c->d_qps)+queue, ((b_page)*cache.n_blocks_per_page), cache.n_blocks_per_page, page_trans);
                //page_addresses[index].store(page_trans, cuda::memory_order_release);
                pages[index].offset = page_trans;
                // while (cache.page_translation[global_page].load(cuda::memory_order_acquire) != page_trans)
                //     __nanosleep(100);
                //miss_cnt.fetch_add(count, cuda::memory_order_relaxed);
                miss_cnt.fetch_add(count, cuda::memory_order_relaxed);
                //new_state = VALID;
                if (write)
                    pages[index].state.fetch_or(DIRTY, cuda::memory_order_relaxed);
                //new_state |= DIRTY;
                //pages[index].state.fetch_or(new_state, cuda::memory_order_relaxed);
                pages[index].state.fetch_xor(DISABLE_BUSY_ENABLE_VALID, cuda::memory_order_release);
                return page_trans;
        ...
        case V_NB:
            if (write && ((read_state & DIRTY) == 0))
                pages[index].state.fetch_or(DIRTY, cuda::memory_order_relaxed);
            //uint32_t page_trans = pages[index].offset.load(cuda::memory_order_acquire);
            uint32_t page_trans = pages[index].offset;
            // while (cache.page_translation[global_page].load(cuda::memory_order_acquire) != page_trans)
            //     __nanosleep(100);
            //hit_cnt.fetch_add(count, cuda::memory_order_relaxed);
            hit_cnt.fetch_add(count, cuda::memory_order_relaxed);
            return page_trans;

            //pages[index].fetch_sub(1, cuda::memory_order_release);
            fail = false;
...
#endif
            read_state = pages[index].state.load(cuda::memory_order_acquire);
        }
    ...
    void memcpy(const uint64_t i, const uint64_t count, T* dest) {
        ...
        if (r != -1) {
            ...
            if (lane == leader) {
                ...
                ctrl = pc->ctrl_counter->fetch_add(1, cuda::memory_order_relaxed) % (pc->n_ctrls);
...
//                //printf("++tid: %llu\tbase: %p  page:%llu\n", (unsigned long long) threadIdx.x, base_master, (unsigned long long) page);
    ...
    void coalesce_page(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write,
        ...
        if (lane == leader) {
            page_cache_d_t* pc = &(r_->cache);
            ctrl = 0;//pc->ctrl_counter->fetch_add(1, cuda::memory_order_relaxed) % (pc->n_ctrls);
...
//                //printf("++tid: %llu\tbase: %p  page:%llu\n", (unsigned long long) threadIdx.x, base_master, (unsigned long long) page);
    ...
    void* acquire_page_(const size_t i, data_page_t*& page_, size_t& start, size_t& end, range_d_t<T>* r_, const size_t page) const {
        ...
        if (r_) {
            ...
            uint64_t gaddr = r_->get_global_address(page);
            page_cache_d_t* pc = &(r_->cache);
            uint32_t ctrl = 0;//pc->ctrl_counter->fetch_add(1, cuda::memory_order_relaxed) % (pc->n_ctrls);
...
//		printf("here\tc: %llu\n", (unsigned long long) count);
        ...
        page = page_ticket->fetch_add(1, cuda::memory_order_relaxed)  % (this->n_pages);
        //page = page_ticket->fetch_add(1, cuda::memory_order_relaxed);
        ...
        bool lock = false;
        uint32_t v = this->cache_pages[page].page_take_lock.load(cuda::memory_order_relaxed);
        //this->page_take_lock[page].compare_exchange_strong(unlocked, LOCKED, cuda::memory_order_acquire, cuda::memory_order_relaxed);
        //not assigned to anyone yet
        if ( v == FREE ) {
            lock = this->cache_pages[page].page_take_lock.compare_exchange_weak(v, LOCKED, cuda::memory_order_acquire, cuda::memory_order_relaxed);
            if ( lock ) {
                this->cache_pages[page].page_translation = global_address;
                //this->cache_pages[page].range_id = range_id;
                //this->page_translation[page].store(global_address, cuda::memory_order_release);
                this->cache_pages[page].page_take_lock.store(UNLOCKED, cuda::memory_order_release);
                fail = false;
            }
        }
        //assigned to someone and was able to take lock
            ...
            if (lock) {
                ...
                expected_state = this->ranges[previous_range][previous_address].state.load(cuda::memory_order_relaxed);

                uint32_t cnt = expected_state & CNT_MASK;
                uint32_t b = expected_state & BUSY;
                if ((cnt == 0) && (b == 0) ) {
                    new_expected_state = this->ranges[previous_range][previous_address].state.fetch_or(BUSY, cuda::memory_order_acquire);
                    if (((new_expected_state & BUSY ) == 0) ) {
                        //while ((new_expected_state & CNT_MASK ) != 0) new_expected_state = this->ranges[previous_range][previous_address].state.load(cuda::memory_order_acquire);
                        if (((new_expected_state & CNT_MASK ) == 0) ) {
                            ...
                            fail = false;
                            this->ranges[previous_range][previous_address].state.fetch_and(CNT_MASK, cuda::memory_order_release);
                        }
                        else { 
                            this->ranges[previous_range][previous_address].state.fetch_and(DISABLE_BUSY_MASK, cuda::memory_order_release);
//if ((j % 1000000) == 0) {
...
//}
                ...
                //this->ranges[previous_range][previous_address].compare_exchange_strong(expected_state, new_state, cuda::memory_order_acquire, cuda::memory_order_relaxed);
...
inline __device__ void enqueue_second(page_cache_d_t* pc, QueuePair* qp, const uint64_t starting_lba, nvm_cmd_t* cmd, const uint16_t cid, const uint64_t pc_pos, const uint64_t pc_prev_head) {
    ...
    do {
        ...
        uint64_t cur_pc_head = pc->q_head->load(cuda::memory_order_relaxed);
        ...
        //if not
        uint64_t qlv = pc->q_lock->load(cuda::memory_order_relaxed);
        //got lock
        if (qlv == 0) {
            qlv = pc->q_lock->fetch_or(1, cuda::memory_order_acquire);
            if (qlv == 0) {
                uint64_t cur_pc_tail;// = pc->q_tail.load(cuda::memory_order_acquire);

                uint16_t sq_pos = sq_enqueue(&qp->sq, cmd, pc->q_tail, &cur_pc_tail);
                uint32_t head, head_;
                uint32_t cq_pos = cq_poll(&qp->cq, cid, &head, &head_);

                pc->q_head->store(cur_pc_tail, cuda::memory_order_release);
                pc->q_lock->store(0, cuda::memory_order_release);
                pc->extra_reads->fetch_add(1, cuda::memory_order_relaxed);
...
inline __device__ void read_data(page_cache_d_t* pc, QueuePair* qp, const uint64_t starting_lba, const uint64_t n_blocks, const unsigned long long pc_entry) {
    ...
    qp->cq.tail.fetch_add(1, cuda::memory_order_acq_rel);
    pc_prev_head = pc->q_head->load(cuda::memory_order_relaxed);
    pc_pos = pc->q_tail->fetch_add(1, cuda::memory_order_acq_rel);
